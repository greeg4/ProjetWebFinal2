...
if(isset($_POST['submit_rech'])) {
	...
	if(trim($titre)!='' && trim($support)!='' && trim($realisateur)!=''){
		$cat=$r->getjeuxall($titre, $support, $realisateur);
	}
	else if(trim($titre)!='' && trim($support)!=''){
		$cat=$r->getjeuxtc($titre, $support);
	}
	else if(trim($titre)!='' && trim($realisateur)!=''){
		$cat=$r->getjeuxtd($titre, $realisateur);
	}
	else if(trim($support)!='' && trim($realisateur)!=''){
		$cat=$r->getjeuxcd($support, $realisateur);
	}
	else if(trim($titre)!=''){
		$cat=$r->getjeuxt($titre);
	}
	else if(trim($support)!=''){
		$cat=$r->getjeuxc($support);
	}
	else if(trim(realisateur)!=''){
		$cat=$r->getjeuxd($realisateur);
	}
    ...
    extract($_GET,EXTR_OVERWRITE);
	  ...
	  {	  
            ...
            else
            {
                ...
                if(trim($idClient)!='') {$_SESSION['form']['idClient']=$idClient;}
			...
			<tr>
                ...
                <td>
                    <?php if(isset($_SESSION['form']['realisateur'])) { ?>
                        <input type="text" name="realisateur" id="dev" value="<?php print $_SESSION['form']['realisateur'];?>"/>
                    <?php
                    }
                    else {
                        ?>
                        <input type="text" name="nomreal" id="dev" placeholder="Realisateur"/>
                        <?php
                    }
                    ?>
                        <div id="error"></div>
