...
if (isset($_GET['submit_dvd'])) {
    ...
    if (trim($Titre_dvd) != '' && trim($Prix_dvd) != '') {
        ...
        if (trim($Titre_dvd) != '') {
            $_SESSION['form']['Titre_dvd'] = $Titre_dvd;
        }
        if (trim($Prix_dvd) != '') {
            $_SESSION['form']['Prix_dvd'] = $Prix_dvd;
        }
    }
}
?>
...
<section id="leform">
    <form id="form_ajout_dvd" action="<?php print $_SERVER['PHP_SELF']; ?>" method="get">
        <fieldset id="Client">
                ...
                <tr>
                    ...
                    <td>
                        <?php if (isset($_SESSION['form']['Titre_dvd'])) { ?>
                            <input type="text" name="Titre_dvd" id="Titre_dvd" value="<?php print $_SESSION['form']['Titre_dvd']; ?>"/>
                            <?php
                        } else {
                            ?>
                            <input type="text" name="Titre_dvd" id="Titre_dvd" placeholder="Titre" required/>
                            <?php
                        }
                        ?>
                    </td>
                ...
                <tr>
                    ...
                    <td>
                        <?php if (isset($_SESSION['form']['Prix_dvd'])) { ?>
                            <input type="number" step="0.01" min="0" name="Prix_dvd" id="Prix_dvd" value="<?php print $_SESSION['form']['Prix_dvd']; ?>"/>
                            <?php
                        } else {
                            ?>
                            <input type="number" step="0.01" min="0" name="Prix_dvd" id="Prix_dvd" placeholder="Prix" required/>
                            <?php
                        }
                        ?>
                    </td>
